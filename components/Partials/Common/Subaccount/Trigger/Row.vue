  ...
  >
    ...
    <td class="text-left">
      <span
        ...
        class="text-xs"
        :class="{
          'text-qwerty-green': isBuy,
          'text-qwerty-red': !isBuy
        }"
      >
      ...
      <span
        v-if="isReduceOnly"
        class="ml-0.5 text-xs text-qwerty-white"
    ...
    <td class="font-mono text-right">
      <span v-if="isMarketOrder" class="text-qwerty-white text-xs">
        {{ $t('trade.market') }}
      </span>
    ...
    <td>
      <div class="flex items-center justify-end gap-1">
        <span class="text-qwerty-white text-xs font-semibold">
          {{ $t('trade.mark_price') }}
        </span>

        <span
          v-if="(isStopLoss && !isBuy) || (isTakeProfit && isBuy)"
          class="text-qwerty-white text-xs font-semibold"
        >
          &le;
        </span>
        <span v-else class="text-qwerty-white text-xs font-semibold">
          &ge;
        </span>
    ...
    <td class="relative text-right pr-3">
      <div class="flex items-center justify-end">
        <NuxtLink
          v-if="false"
          class="cursor-pointer text-qwerty-white mr-6"
