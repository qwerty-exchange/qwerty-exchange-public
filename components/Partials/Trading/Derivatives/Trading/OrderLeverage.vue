...
const background = computed(() => {
  ...
  const min = 1
  const max = Number(maxLeverageAvailable.value)
  const val = ((Number(leverage.value) - min) * 100) / (max - min)
  return (
    'linear-gradient(to right, #f6be00 0%, #f6be00 ' +
    val +
    '%, #2F313A ' +
    val +
    '%, #2F313A 100%)'
...
<template>
  <div class="pt-1">
    <div>
      <h3 class="text-xs text-qwerty-white">
    ...
    <div class="range-wrap flex items-center relative select-none gap-2">
      ...
      <div class="relative max-h-6">
        ...
        />
        <span
          class="absolute top-0 right-0 text-xs text-qwerty-white mt-1.5 mr-1.5"
        >
          x
        </span>
      </div>
...
.input-base.leverage-input {
  @apply font-mono ml-2 w-16 py-1 h-auto text-sm text-right rounded-lg appearance-none text-qwerty-white;
}
