...
<template>
  <div
    class="flex justify-start text-qwerty-white gap-4 text-xs px-3 py-2 bg-qwerty-shade2 items-center hover:bg-qwerty-shade3"
    :data-cy="`markets-table-row-${market.ticker}`"
  >
    <div
      class="text-qwerty-white w-6 h-6 flex items-center justify-center rounded-full hover:bg-qwerty-shade3 hover:bg-opacity-10 cursor-pointer"
    ...
    >
      <div class="flex flex-col">
        <span
          class="font-semibold text-qwerty-white"
          data-cy="markets-ticker-table-data"
        >
          {{ market.ticker }}
        </span>
        <span
          class="text-qwerty-white tracking-wide mt-1 font-mono"
