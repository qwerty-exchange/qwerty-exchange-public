...
<template>
  <div>
    ...
    >
      <span class="text-qwerty-primary text-sm font-medium cursor-pointer">
        {{ $t('account.trade') }}
      </span>
    </NuxtLink>
    ...
    <BaseDropdown
      v-else
      popper-class="rounded-lg flex flex-col flex-wrap text-xs absolute max-w-52 bg-qwerty-shade2 shadow-dropdown"
    >
      <template #default>
        <div
          class="rounded flex items-center justify-center w-auto h-auto cursor-pointer"
      ...
      <template #content>
        <div class="flex flex-col py-2">
          <NuxtLink
            ...
            :key="market.slug"
            class="px-4 py-2 font-semibold text-sm uppercase cursor-pointer text-qwerty-white hover:text-qwerty-primary"
            :to="market.route"
          >
