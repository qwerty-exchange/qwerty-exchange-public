...
<template>
  <div class="flex flex-col gap-4 md:grid md:grid-cols-4 md:items-center py-4">
    ...
    >
      ...
      >
        <span
          class="text-sm"
          :class="{
            'text-qwerty-primary': !showFilters,
            'text-qwerty-white': showFilters
          }"
        >
          {{ $t('account.filters') }}
        </span>
      ...
      <button
        ...
        class="bg-qwerty-red rounded-lg px-3 h-8 flex md:hidden items-center justify-center w-auto"
    ...
    <div class="col-span-1 items-center justify-end gap-6 hidden md:flex">
      <button
        v-if="positionStore.subaccountPositions.length"
        class="bg-qwerty-red rounded-lg px-3 h-8 flex items-center justify-center w-full md:w-auto"
        data-cy="trading-account-positions-table-cancel-all-button"
        @click="handleCloseAllPositions"
      >
        <span class="text-xs text-qwerty-white">
