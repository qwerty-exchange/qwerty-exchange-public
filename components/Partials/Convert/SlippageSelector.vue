...
const appIconColorClass = computed<string>(() => {
  ...
  return isOpen.value ? 'text-qwerty-primary' : 'text-qwerty-white'
})
...
<template>
  ...
  >
    ...
    >
      <BaseIcon
        name="gear"
        class="h-5 w-5 hover:text-qwerty-primary"
        :class="appIconColorClass"
        @click="toggleSlippageDropdown"
      />

      <template #popper>
        <div ref="popperRef" class="p-4 bg-gray-800 text-qwerty-white">
          ...
          <div class="flex items-center gap-2 max-xs:flex-wrap">
            <div class="flex items-center gap-2 max-xs:w-full">
              ...
              >
                <template #default="{ active }">
                  <AppButton
                    sm
                    class="w-full border-qwerty-primary border"
                    :class="[active ? 'bg-qwerty-primary rounded' : 'rounded']"
                  >
            ...
            >
              ...
              <template #addon>
                <span class="text-qwerty-white">%</span>
              </template>
            </AppInputNumeric>
          </div>
