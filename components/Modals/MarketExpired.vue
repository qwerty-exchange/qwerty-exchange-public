...
<template>
  <AppModal :show="isModalOpen" sm hide-close-button @modal:closed="close">
    ...
    <div v-if="market" class="flex justify-between mt-4">
      ...
      <p class="text-sm text-qwerty-white mt-4">
        {{ $t('marketExpired.expiredNote') }}
      </p>
      <p class="text-sm text-qwerty-white mt-2">
      ...
      <div class="mt-6 flex items-center justify-center gap-2">
        ...
        <NuxtLink :to="{ name: 'activity' }" target="_blank">
          <AppButton
            class="border-qwerty-primary text-qwerty-primary hover:bg-qwerty-primary hover:text-qwerty-primary"
          >
