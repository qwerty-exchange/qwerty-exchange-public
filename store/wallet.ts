...
export const useWalletStore = defineStore('wallet', {
  ...
  actions: {
    ...
    async onConnect () {
      const walletStore = useWalletStore()
      const accountId = await authApi
        .fetchAccount(walletStore.injectiveAddress)
        .then(value => value.baseAccount.accountNumber.toString())
        .catch(error => {
    ...
    async onConnect2 (mode: 'classic' | 'modern' = 'classic', timeLife = 0) {
      const accountStore = useAccountStore()
      const walletStore = useWalletStore()

      if (mode === 'modern') {
        const result = await msgBroadcastClient.createAccount(timeLife!)
        walletStore.$patch({
          sso: {
            token: result.token,
